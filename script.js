...
function paintEveningGlow(){
  const cx = W * 0.22;
  const cy = H * 0.62;
  const maxR = Math.max(W, H) * 0.60;

  const g = ctx.createRadialGradient(cx, cy, 20, cx, cy, maxR);
  g.addColorStop(0.00, "rgba(255, 138, 61, 0.26)");
  g.addColorStop(0.25, "rgba(255, 79, 136, 0.14)");
  g.addColorStop(0.58, "rgba(255, 214, 143, 0.10)");
...
function paintGlowDust(){
  ...
  for (const p of glowDust){
    if (!prefersReducedMotion){
      ...
      p.a = Math.max(0.03, Math.min(0.26, p.a));
      p.x += p.vx;
      p.y += p.vy;

      if (p.y < H * 0.12) { p.y = H + rand(10, 140); p.x = rand(0, W); }
      if (p.x < -20) p.x = W + 20;
      if (p.x > W + 20) p.x = -20;
    }
...
function paintNightGlow(){
  ...
  g.addColorStop(0.35, "rgba(197,155,255,0.06)");
